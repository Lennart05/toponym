...
The plot displays all locations which end in -itz in Germany, their total frequency (2170), and stores the data in the global environment.
...
For the parameter `global`: if the current setting is `TRUE`, matches from `top()` and strings from `topComp()` will be saved in the global environment; if the current setting is `FALSE`, the results will not be saved.
