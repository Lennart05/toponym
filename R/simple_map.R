...
#' @param x numeric. Latitude of locations filtered by \code{getCoordinates()}
#' @param y numeric. Longitude of locations filtered by \code{get.coordinates()}
#' @param cc character string. Country code of locations filtered by \code{getCoordinates()}
#' @param matches character string. string of each entry filtered by \code{getCoordinates()}
...
simpleMap <- function(coordinates, color, strings, regions = 0, plot, ratio_string, fq, lons, lats) {

  x <- coordinates[[1]]
  y <- coordinates[[2]]
  cc <- coordinates[[3]]
  matches <- coordinates[[4]]
  ...
  p <- ggplot() +
    ...
    {if(nchar(ratio_string) > 0 && nchar(fq) > 0)labs(title = paste(strings, ratio_string, fq, collapse = " "))} # extended legend if created with topCompOut()
