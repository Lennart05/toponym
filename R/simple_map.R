...
#' @param ratio_string character string. Ratio of occurrences in each polygon returned by \code{top.candidates()}.
...
simple.map <- function(x, y, cc, color, strings, plot, ratio_string, fq) {
  ...
  md <- cbind(as.numeric(x), as.numeric(y), cc) %>% # creates df out of x and y coordinates
    as.data.frame() %>%
  ...
  p <- ggplot() +
    ...
    labs(x = "longitude", y = "latitude", color = "country", title = paste(strings, length(md[,1]))) +   # legend only with string & frequency
