...
simple.map <- function(x, y, cc, matches, color, strings, regions = FALSE, plot, ratio_string, fq) {
  ...
  country_names <- country.names(country_code = unique(cc)) # converts country codes into country names required by ne_states
  map <- ne_states(country = country_names, returnclass = "sf") # gets map of specified countries with domestic borders from pkg "rnaturalearth"
  }
